...
- **Edge Functions cockpit** : `cockpit-invoice-pdf` (verify_jwt + double check email ; pdf-lib + qrcode via esm.sh ; A4 + QR Swiss Payments Code v2.2, adresse type S, réf NON ; charge les données via PostgREST avec le JWT user pour respecter la RLS). `cockpit-umami-stats` existe encore mais est ORPHELINE (plus appelée — à supprimer du dashboard à l'occasion, pas de tool MCP delete).
